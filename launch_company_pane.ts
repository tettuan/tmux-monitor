...
async function launchCompanyPane() {
  ...
  try {
    ...
    const createResult = await executor.executeTmuxCommand("tmux split-window -h");
    if (!createResult.ok) {
    ...
    const paneIdResult = await executor.executeTmuxCommand("tmux display-message -p '#{pane_id}'");
    if (!paneIdResult.ok) {
    ...
    const titleResult = await executor.executeTmuxCommand(`tmux select-pane -t ${paneId} -T "Company"`);
    if (!titleResult.ok) {
    ...
    const startResult = await executor.executeTmuxCommand(`tmux send-keys -t ${paneId} "cld" Enter`);
    if (!startResult.ok) {
      throw new Error(`Failed to start Claude Code: ${startResult.error.message}`);
    }
